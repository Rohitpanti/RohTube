...
//Update we arre gone Verfiy the id if same we update or else we throw error
...
//Adding View Count on a Video
export const addView=async(req,res,next)=>{
    try{
        await Video.findByIdUpdate(
            req.params.id,{
            $inc:{views:1}
            }
        )
        res.status(200).json("The views has ben incresed.")
    }catch(err){
        next(err);
    }
}
